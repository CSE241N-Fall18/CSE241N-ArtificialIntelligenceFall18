...
def train_func(train_list_words, train_list_tags):
	...
	for sentence in train_list_words:
		for word in sentence:
			if word_count[word] <= 1:
				train_list_words[train_list_words.index(sentence)][sentence.index(word)] = 'unk'
			else:
				continue
	
	for sentence, tags in zip(train_list_words, train_list_tags):
		for word, tag in zip(sentence, tags):
			if word not  in dict2_word_tag:
				dict2_word_tag[word] = {}
	...
	for word, tags in dict2_word_tag.items():
		su = 0
		for tag, value in tags.items():
			su += value

		for a in dict2_word_tag[word]:
			dict2_word_tag[word][a] /= su
	

	dict2_tag_follow_tag['#'] = {}

	for first_tag in train_list_tags:
		if first_tag[0] not in dict2_tag_follow_tag['#']:
			dict2_tag_follow_tag['#'][first_tag[0]] = 1
		else:
			dict2_tag_follow_tag['#'][first_tag[0]] += 1
	for tag_list in train_list_tags:
		for index in range(len(tag_list) - 1):
			if tag_list[index] not in dict2_tag_follow_tag:
				dict2_tag_follow_tag[tag_list[index]] = {}
			if tag_list[index + 1] not in dict2_tag_follow_tag[tag_list[index]]:
				dict2_tag_follow_tag[tag_list[index]][tag_list[index + 1]] = 0
			dict2_tag_follow_tag[tag_list[index]][tag_list[index + 1]] += 1
	
	
	
	for first_tag, follows in dict2_tag_follow_tag.items():
		su = 0
		for tag, value in follows.items():
			su += value

		for a in dict2_tag_follow_tag[first_tag]:
			dict2_tag_follow_tag[first_tag][a] /= su
...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
	...
	tag_list = list(dict2_tag_follow_tag.keys())
	tag_list.remove('#')
	...
	for sentence in test_words:
		for word in sentence:
			if word not in dict2_word_tag:
				test_words[test_words.index(sentence)][sentence.index(word)] = 'unk'
				#word = 'unk'
				#print(word)
	...
	for sentence in test_words:
			...
			if len(backtrace) == 0:
				for tag in tag_list:
					if tag not in dict2_word_tag[word]:
						...
						try:
							temp.append(dict2_tag_follow_tag['#'][tag]*dict2_word_tag[word][tag])
						except:
							temp.append(0) 
				backtrace.append(tag_list[temp.index(max(temp))])
			else:
				for tag in tag_list:
					if tag not in dict2_word_tag[word]:
						temp.append(0)
						...
						try:
							temp.append(dict2_tag_follow_tag[backtrace[len(backtrace) - 1]][tag]*dict2_word_tag[word][tag])
						except:
							temp.append(0)
		...
		output_test_tags.append(backtrace)

...
#NEEDS TO BE FILLED!
def train_func(train_li_words, train_li_tags):
...
num_words_train=len(train_li_tags)
         ...
         while counter_trans < len(di_transition_probs) and counter_emis < len(di_emission_probs):
            ...
            if tag_tr < tag_em:
                ...
                if prod_prob > max_prod_prob:
                    ...
                    output_test_tags[i] = tag_tr
                    #print "i=",i," and output=",output_li[i]
                counter_trans+=1
                counter_emis+=1    
    

    if output_test_tags[i]=='': #In case there are no matching entries between the transition tags and emission tags, we choose the most frequent emission tag
          output_test_tags[i] = max(di_emission_probs,key=itemgetter(1))[0] 

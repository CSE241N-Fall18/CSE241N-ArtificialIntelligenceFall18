...
def train_func(train_list_words, train_list_tags):
    ...
    starttag = "<S>"   #naming the start tag

    def filtering(seq, foundd=None):     #function to checkwords-tag 
        if foundd is None:
            foundd = []
        for s in seq:
    ...
    chk_count = filtering(train_list_tags)
    count_tags = Counter(chk_count)     #just checking
    

    # Calculating intial probablity of each word
    initialprob = {}
    tempStartTgList = [tagsentance[0] for tagsentance in train_list_tags]
    for tgSent in train_list_tags:
        initialprob[tgSent[0]] = tempStartTgList.count(tgSent[0]) / count_tags[tgSent[0]]
   

    # Populating the tag follow tag dictionary and the word given tag dictionary
    assert len(train_list_words) == len(train_list_tags) #to check the valdity of the list we obtained is balanced or not
    train_lst_tag_flat = filtering(train_list_tags)
    train_lst_wrd_flat = filtering(train_list_words)
    for indx, (init_tag, wrd) in enumerate(zip(train_lst_tag_flat, train_lst_wrd_flat)):
        wrd = wrd.lower()
        if init_tag not in dict2_tag_follow_tag:      # initializing the tag follow tag dictionary
            dict2_tag_follow_tag[init_tag] = {}
        if wrd not in dict2_word_tag:           # initializing the word for a tag dictionary
            dict2_word_tag[wrd] = {}
        if indx < len(train_lst_tag_flat) - 2 and indx < len(train_lst_wrd_flat) - 2:#sub 2 to neglect 1st nd last value
            # below we count the tag after current tag
            nxtTg = train_lst_tag_flat[indx + 1]
            if nxtTg in dict2_tag_follow_tag[init_tag]:
                dict2_tag_follow_tag[init_tag][nxtTg]=1+ dict2_tag_follow_tag[init_tag][nxtTg] 
            else:
                dict2_tag_follow_tag[init_tag][nxtTg] = 1
            # below we count the emitter tag for current word
            if init_tag in dict2_word_tag[wrd]:
                dict2_word_tag[wrd][init_tag]=1+dict2_word_tag[wrd][init_tag]
            else:
                dict2_word_tag[wrd][init_tag]=1

    # now generating probabilities from counts
    for init_tag in dict2_tag_follow_tag:
        for followTag in dict2_tag_follow_tag[init_tag]:
            dict2_tag_follow_tag[init_tag][followTag]=dict2_tag_follow_tag[init_tag][followTag]/count_tags[init_tag]        # this will give the probabilities
    for wrds in dict2_word_tag:
        for emitTag in dict2_word_tag[wrds]:
            dict2_word_tag[wrds][emitTag] /=count_tags[emitTag]
    # appending the starting probabilities
    dict2_tag_follow_tag[starttag] =initialprob
...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
    ...
    starttag = "<S>" #----
    states = ["C", "D", "E", "F", "I", "J", "L", "M", "N", "P", "R", "S", "T", "U","dictionary", "W", "###", ",", ".", ":", "-", "'", "$"]
    nil = 0
    delta=1
    BASIC = 0.000001e-19    #we take it from andrew ng notes -----

    for Wlst in test_words:
        output_list = []
        # now implement for each list
        dictionary = [{}]        # list of dictionaries-------------
        for i in states:

            init_prob = dict2_tag_follow_tag[starttag][i] if i in dict2_tag_follow_tag[starttag] else nil
            emissionprob = dict2_word_tag[Wlst[0].lower()][i] if Wlst[0].lower() in dict2_word_tag and i in dict2_word_tag[Wlst[0].lower()] else BASIC

            dictionary[0][i] = {"prob": init_prob * emissionprob, "prev": None}
        # forward probability calculations
        for t in range(1, len(Wlst)):
            dictionary.append({})
            for i in states:
                probab_prod = []
                for prev_st in states:
                    trans_prob = dict2_tag_follow_tag[prev_st][i] if prev_st in dict2_tag_follow_tag and i in dict2_tag_follow_tag[prev_st] else nil
                    probab_prod.append(dictionary[t-1][prev_st]["prob"] * trans_prob)
                max_tr_prob = max(probab_prod)

                for prev_st in states:
                    trans_prob = dict2_tag_follow_tag[prev_st][i] if prev_st in dict2_tag_follow_tag and i in dict2_tag_follow_tag[prev_st] else nil
                    if dictionary[t-1][prev_st]["prob"] * trans_prob == max_tr_prob:
                        emissionprob = dict2_word_tag[Wlst[t].lower()][i] if Wlst[t].lower() in dict2_word_tag and i in dict2_word_tag[Wlst[t].lower()] else BASIC
                        max_prob = max_tr_prob * emissionprob
                        dictionary[t][i] = {"prob": max_prob, "prev": prev_st}
                        break

        # The highest probability
        max_prob = max(value["prob"] for value in dictionary[-1].values())
        previous = None
        # Get most probable state and its backtrack
        for statr_prob, data in dictionary[-1].items():
            if data["prob"] == max_prob:
                output_list.append(statr_prob)
                previous = statr_prob
                break
        # Follow the backtrack till the first observation
        for t in range(len(dictionary) - 2, -1, -1):
            output_list.insert(0, dictionary[t + 1][previous]["prev"])
            previous = dictionary[t + 1][previous]["prev"]

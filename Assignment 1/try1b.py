...
def train_func(train_list_words, train_list_tags):
    ...
    dict2_tag_follow_tag[stag] =initialprob
...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
    ...
    stag = "<S>" #----
    states = ["C", "D", "E", "F", "I", "J", "L", "M", "N", "P", "R", "S", "T", "U","dictnry", "W", "###", ",", ".", ":", "-", "'", "$"]
    ZERO = 0
    delta=1
    BASIC = 0.0001e-22     #we take it from andrew ng notes -----

    for wordLst in test_words:
        output_list = []
        # now implement for each list
        dictnry = [{}]        # list of dictionaries-------------
        for i in states:

            init_prob = dict2_tag_follow_tag[stag][i] if i in dict2_tag_follow_tag[stag] else ZERO
            eprob = dict2_word_tag[wordLst[0].lower()][i] if wordLst[0].lower() in dict2_word_tag and i in dict2_word_tag[wordLst[0].lower()] else BASIC

            dictnry[0][i] = {"prob": init_prob * eprob, "prev": None}
        # forward probability calculations
        for t in range(1, len(wordLst)):
            dictnry.append({})
            for st in states:
                probab_prod = []
                for prev_st in states:
                    ...
                    probab_prod.append(dictnry[t-1][prev_st]["prob"] * trans_prob)
                max_tr_prob = max(probab_prod)

                for prev_st in states:
                    trans_prob = dict2_tag_follow_tag[prev_st][st] if prev_st in dict2_tag_follow_tag and st in dict2_tag_follow_tag[prev_st] else ZERO
                    if dictnry[t-1][prev_st]["prob"] * trans_prob == max_tr_prob:
                        eprob = dict2_word_tag[wordLst[t].lower()][st] if wordLst[t].lower() in dict2_word_tag and st in dict2_word_tag[wordLst[t].lower()] else BASIC
                        max_prob = max_tr_prob * eprob
                        dictnry[t][st] = {"prob": max_prob, "prev": prev_st}
                        break

        # The highest probability
        max_prob = max(value["prob"] for value in dictnry[-1].values())
        previous = None
        # Get most probable state and its backtrack
        for statr_prob, data in dictnry[-1].items():
            if data["prob"] == max_prob:
                output_list.append(statr_prob)
                previous = statr_prob
                break
        # Follow the backtrack till the first observation
        for t in range(len(dictnry) - 2, -1, -1):
            output_list.insert(0, dictnry[t + 1][previous]["prev"])
            previous = dictnry[t + 1][previous]["prev"]

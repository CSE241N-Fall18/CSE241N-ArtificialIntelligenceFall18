...
def train_func(train_list_words, train_list_tags):
    ...
    words, tags = train_list_words.copy(), tags_list_train.copy()

    counter = {}  # dictionary for counting occurences of tags

    for i in range(len(words)):
        for j in range(len(words[i])):
            if tags[i][j] not in counter:
                counter[tags[i][j]] = 1
            else:
                counter[tags[i][j]] += 1
    counter["*"] = len(words)
    ...
    for i in range(len(words)):
        for j in range(len(words[i])):
            if j == 0:
                if '*' not in dict2_tag_follow_tag.keys():
                    dict2_tag_follow_tag['*'] = dict()
                if tags[i][j] not in dict2_tag_follow_tag['*'].keys():
                    dict2_tag_follow_tag['*'][tags[i][j]] = 1.0 / counter["*"]
                else:
                    dict2_tag_follow_tag['*'][tags[i][j]] += 1.0 / counter["*"]
            else:
                if tags[i][j - 1] not in dict2_tag_follow_tag:
                    dict2_tag_follow_tag[tags[i][j - 1]] = dict()
                if tags[i][j] not in dict2_tag_follow_tag[tags[i][j - 1]]:
                    dict2_tag_follow_tag[tags[i][j - 1]][tags[i][j]] = 1.0 / counter[tags[i][j - 1]]
                else:
                    dict2_tag_follow_tag[tags[i][j - 1]][tags[i][j]] += 1.0 / counter[tags[i][j - 1]]
    ...
    for i in range(len(words)):
        for j in range(len(words[i])):
            if words[i][j] not in dict2_word_tag:
                dict2_word_tag[words[i][j]] = {}
            if tags[i][j] not in dict2_word_tag[words[i][j]]:
                dict2_word_tag[words[i][j]][tags[i][j]] = 1.0 / counter[tags[i][j]]
            else:
                dict2_word_tag[words[i][j]][tags[i][j]] += 1.0 / counter[tags[i][j]]
...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
    ...
    output_test_tags = []  # list of list of predicted tags, corresponding to the list of list of words in Test set (
    # test_words input to this function)

    #      *** WRITE YOUR CODE HERE ***

    # states or tags list
    states = [
        "`", "D", "E", "F", "I", "J", "L", "M", "C", "N", "R", "S", "T", "U", "V", "W", ",", ".", ":", "-", "'", "$",
        'P'
    ]
    for i in range(len(test_words)):
        pi = []  # probability matrix
        bp = []  # backpointer matrix for backtracking
        # initializing matrices
        for j in range(len(test_words[i])):
            pi.append([0 for k in states])
            bp.append([-1 for k in states])
        for j in range(len(states)):
            if states[j] in dict2_tag_follow_tag['*'] and test_words[i][0] in dict2_word_tag and states[j] in dict2_word_tag[test_words[i][0]]:
                pi[0][j] = dict2_tag_follow_tag['*'][states[j]] * dict2_word_tag[test_words[i][0]][states[j]]
                bp[0][j] = j
            else:
                # taking zero for tags not occuring after previous tag according to training data
                pi[0][j] = 0.0
                bp[0][j] = -1
        flag = 0
        if test_words[i][0] not in dict2_word_tag:  # for word not occurring in training data
            flag = 1
        for j in range(1, len(test_words[i])):
            for k in range(len(states)):
                ma = -1
                val = 0
                for l in range(len(states)):
                    if test_words[i][j] in dict2_word_tag and states[k] in dict2_tag_follow_tag[states[l]] and states[
                        k] in dict2_word_tag[test_words[i][j]]:
                        if val < pi[j - 1][l] * dict2_tag_follow_tag[states[l]][states[k]] * dict2_word_tag[test_words[i][j]][states[k]]:
                            val = pi[j - 1][l] * dict2_tag_follow_tag[states[l]][states[k]] * dict2_word_tag[test_words[i][j]][states[k]]
                            ma = l
                        elif j == 1 and flag:
                            # for not seen word occuring at first place
                            if val < dict2_tag_follow_tag[states[l]][states[k]] * dict2_word_tag[test_words[i][j]][states[k]]:
                                val = dict2_tag_follow_tag[states[l]][states[k]] * dict2_word_tag[test_words[i][j]][states[k]]
                                ma = l
                if val != 0:
                    pi[j][k] = val
                    bp[j][k] = ma
                    ...
                    pi[j][k] = 0.0
                    bp[j][k] = -1
                if test_words[i][j] not in dict2_word_tag:  # for not seen word occuring in middle
                    pi[j][k] = 1.0
                    bp[j][k] = -1
        z = [-1 for i in test_words[i]]
        for j in range(len(states)):
            if z[len(test_words[i]) - 1] == -1 and pi[len(test_words[i]) - 1][j] != 1.0 and pi[len(test_words[i]) - 1][
                j] != 0.0:
                z[len(test_words[i]) - 1] = j
            elif pi[len(test_words[i]) - 1][j] != 0.0 and pi[len(test_words[i]) - 1][j] != 0.0 and pi[len(test_words[i]) - 1][j] > pi[len(test_words[i]) - 1][z[len(test_words[i]) - 1]]:
                z[len(test_words[i]) - 1] = j
        tags = ["" for i in test_words[i]]
        if z[len(test_words[i]) - 1] == -1:
            tags[len(test_words[i]) - 1] = "N"
        else:
            tags[len(test_words[i]) - 1] = states[z[len(test_words[i]) - 1]]
        for j in range(len(test_words[i]) - 1, 0, -1):
            if z[j] == -1:
                for k in range(len(states)):
                    if z[j - 1] == -1 and pi[j - 1][k] != 1.0 and pi[j - 1][k] != 0.0:
                        z[j - 1] = k
                    elif pi[j][k] != 0.0 and pi[j][k] != 1.0 and pi[j - 1][k] > pi[j - 1][z[j - 1]]:
                        z[j - 1] = k
                if z[j - 1] == -1:
                    tags[j - 1] = "N"
                else:
                    tags[j - 1] = states[z[j - 1]]
            else:
                for k in range(len(states)):
                    if pi[j - 1][k] != 1.0:
                        z[j - 1] = bp[j][z[j]]

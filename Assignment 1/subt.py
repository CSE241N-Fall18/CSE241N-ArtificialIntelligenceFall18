...
def train_func(train_list_words, train_list_tags):
	...
	INPUT: The nested list of words and corresponding nested list of tags from the TRAINING set. This passing
	of correct lists and calling the function
	has been done for you. You only need to write the code for filling in the below dictionaries.
	(created with bigram-HMM in mind)
	OUTPUT: The two dictionaries

	HINT: Keep in mind the boundary case of the starting POS tag. You may have to choose 
	(and stick with) some starting POS tag to compute bigram probabilities
	...
	"""
	tags = ["C","D","E","F","I","J","L","M","N","P","R","S","T","U","V","W","###",",",".",":","-","'","$","`","#"]
	for t in tags:
		dict2_tag_follow_tag[t]={}

		for _ in tags:
			dict2_tag_follow_tag[t][_]=0

	for sentence in train_list_tags:
		for word in sentence:
			#if sentence.index(word)<sentence.length()-1:
			if sentence.index(word)==0:
				dict2_tag_follow_tag["###"][word]=dict2_tag_follow_tag["###"][word]+1
			else:
				dict2_tag_follow_tag[sentence[sentence.index(word)-1]][word]=dict2_tag_follow_tag[sentence[sentence.index(word)-1]][word]+1

	for _ in dict2_tag_follow_tag.keys():
		count = 0 
		for i in dict2_tag_follow_tag[_].keys():
			count = count + dict2_tag_follow_tag[_][i]

		for i in dict2_tag_follow_tag[_].keys():
			dict2_tag_follow_tag[_][i] = dict2_tag_follow_tag[_][i]/float(count)
	...
	for sentence in train_list_words:
		for word in sentence:
			if word not in dict2_word_tag.keys():
				dict2_word_tag[word]={}
				dict2_word_tag[word][train_list_tags[train_list_words.index(sentence)][sentence.index(word)]]=1.0
			else:
				if train_list_tags[train_list_words.index(sentence)][sentence.index(word)] not in dict2_word_tag[word].keys():
					dict2_word_tag[word][train_list_tags[train_list_words.index(sentence)][sentence.index(word)]]=1.0
				else:
					dict2_word_tag[word][train_list_tags[train_list_words.index(sentence)][sentence.index(word)]] = \
					dict2_word_tag[word][train_list_tags[train_list_words.index(sentence)][sentence.index(word)]]+1.0


	dict2_word_tag["unk"]={}
	l = list(dict2_word_tag.keys())
	for _ in l:
		count = 0 
		for i in dict2_word_tag[_].keys():
			count = count + dict2_word_tag[_][i] 

		if count == 1:
			key = list(dict2_word_tag[_].keys())[0]
			del dict2_word_tag[_]
			dict2_word_tag["unk"][key] =dict2_word_tag["unk"].get(key,0)+1 

			
	for _ in dict2_word_tag.keys():
		count = 0 
		for i in dict2_word_tag[_].keys():
			count = count + dict2_word_tag[_][i]
		...
		for i in dict2_word_tag[_].keys():
			dict2_word_tag[_][i] = dict2_word_tag[_][i]/float(count)
...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
	...
	for i in test_words:
		...
		for _ in tags:
			...
			temp2 = []
			try:
				temp.append(dict2_word_tag[i[0]][_]*(dict2_tag_follow_tag["###"][_]+dict2_tag_follow_tag["."][_]))
				temp2.append(0)
			except:
				temp.append(dict2_word_tag["unk"].get(_,0)*(dict2_tag_follow_tag["###"][_]+dict2_tag_follow_tag["."][_]))
				temp2.append(0)
		...
		for j in range(1,len(i)):
			for s in range(len(tags)):
				state = tags[s]
				if i[j] not in dict2_word_tag.keys():
					ax, bx = reduce(lambda x,y : x if x[1]>y[1] else y ,[( a,t1[a][j-1] *dict2_tag_follow_tag[tags[a]][state] *\
						dict2_word_tag["unk"].get(state,0.000001)) for a,b in enumerate(tags)  ])
				else:
					ax, bx = reduce(lambda x,y : x if x[1]>y[1] else y ,[( a,t1[a][j-1] * dict2_tag_follow_tag[tags[a]][state] *\
						dict2_word_tag.get(i[j],{}).get(state,0.000001)) for a,b in enumerate(tags) ]) 
		...
		# 		for s in range(len(tags)):
		# 			state = tags[s]
		# 			ax, bx = reduce(lambda x,y : x if x[1]>y[1] else y ,[( a,t1[a][j-1] *dict2_tag_follow_tag[tags[a]][state]) for a,b in enumerate(tags)  ]) 
		...
		# 			state = tags[s]
		# 			ax, bx = reduce(lambda x,y : x if x[1]>y[1] else y ,[( a,t1[a][j-1] * dict2_tag_follow_tag[tags[a]][state] *\
		# 				dict2_word_tag.get(i[j],{}).get(state,0)) for a,b in enumerate(tags)  ])
		# 			t1[s].append(bx)
		# 			t2[s].append(ax)

		
		output = []
		xx,yy = reduce(lambda x,y : x if x[1]>y[1] else y ,[(a,t1[a][len(i)-1]) for a,b in enumerate(tags)])

		output.append(tags[xx])
		z=xx
		for _ in reversed(range(1,len(i))):
			z = t2[z][_]
			output.append(tags[z])

		output_test_tags.append(list(reversed(output)))

	# END OF YOUR CODE
	print(output_test_tags[2])

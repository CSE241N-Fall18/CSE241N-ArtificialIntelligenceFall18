...
def train_func(train_list_words, train_list_tags):
	...
	tt=Counter(itertools.chain.from_iterable(train_list_words))

	dict2_tag_follow_tag['*']={}
	for snt in train_list_tags:
		dict2_tag_follow_tag['*'][snt[0]]=dict2_tag_follow_tag['*'].get(snt[0],0)+1
		for j in range(len(snt)-1):
			dict2_tag_follow_tag.setdefault(snt[j],{})
			dict2_tag_follow_tag[snt[j]][snt[j+1]]=dict2_tag_follow_tag[snt[j]].get(snt[j+1],0)+1

	for i in dict2_tag_follow_tag.keys():
		sm=sum(dict2_tag_follow_tag[i].values())
		for j in dict2_tag_follow_tag[i].keys():
			dict2_tag_follow_tag[i][j]=float(dict2_tag_follow_tag[i][j])/sm



	for i in range(len(train_list_words)):
		for j in range(len(train_list_words[i])):
			if tt[train_list_words[i][j]]==1:
				tt[train_list_words[i][j]]=0
				train_list_words[i][j]='<UNK>'
			dict2_word_tag.setdefault(train_list_words[i][j],{})
			dict2_word_tag[train_list_words[i][j]][train_list_tags[i][j]]= dict2_word_tag[train_list_words[i][j]].get(train_list_tags[i][j],0)+1

	for i in dict2_word_tag.keys():
		sm=sum(dict2_word_tag[i].values())
		for j in dict2_word_tag[i].keys():
			dict2_word_tag[i][j]=float(dict2_word_tag[i][j])/sm
...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
	...
	states=["N","V","D","C","E","F","I","J","L","M","P","R","S","T","U","W",",",".",":","-","'","$","`","#"]

	#      *** WRITE YOUR CODE HERE ***

	global tt

	for snt in test_words:
		sz=len(snt)
		dp=[[0 for j in range(len(states))] for i in range(sz+1)]
		argmx=[[0 for j in range(len(states))] for i in range(sz+1)]
		tword=snt[0]
		if tt.get(snt[0],0)==0:
			tword='<UNK>'
		for j in range(len(states)):
			dp[1][j]=dict2_tag_follow_tag['*'].get(states[j],0.00001)*dict2_word_tag.get(tword,{}).get(states[j],0.00001)

		for i in range(2,sz+1):
			tword=snt[i-1]
			if tt.get(snt[i-1],0)==0:
				tword='<UNK>'
			for j in range(len(states)):
				for k in range(len(states)):
					temp=dp[i][j]
					dp[i][j]=max(dp[i][j],dp[i-1][k]*dict2_tag_follow_tag[states[k]].get(states[j],0.00001))
					if(temp!=dp[i][j] and dp[i][j]!=0):
						argmx[i][j]=k
				dp[i][j]*=dict2_word_tag[tword].get(states[j],0)

		tagged=['' for i in range(sz+1)]
		tagged[sz]=states[dp[sz].index(max(dp[sz]))]
		for i in range(sz,1,-1):
			tagged[i-1]=states[argmx[i][states.index(tagged[i])]]

		output_test_tags.append(tagged[1:])

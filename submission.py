...
def train_func(train_list_words, train_list_tags):
	...
	for i in range(len(train_list_words)):
		for j in range(len(train_list_words[i])):
			if(word_count[train_list_words[i][j]]==1):
				train_list_words[i][j]="UNK"
			if train_list_words[i][j]=="UNK":
				unk_count+=1


	tag_count['*'] = len(train_list_tags)

	#number of each word-tag pairs
	for i in range(len(train_list_words)):
		w_line = train_list_words[i]
		t_line = train_list_tags[i]
		prev = '*'
		for j in range(len(w_line)):
			word = w_line[j]
			tag = t_line[j]
			...
			#number of each tag-tag pairs
			if prev in dict2_tag_follow_tag:
				if tag in dict2_tag_follow_tag[prev]:
					dict2_tag_follow_tag[prev][tag]+=1
				else:
					dict2_tag_follow_tag[prev][tag]=1
			else:
	...
	for word in dict2_word_tag:
	    for tag in dict2_word_tag[word]:
	    	if word=="UNK":
	    		dict2_word_tag[word][tag]/=unk_count*1.0
	    	else:
	    		dict2_word_tag[word][tag]/=word_count[word]*1.0
	
	#calculate transition probabilities 	<<starting tag???>>
	for prev in dict2_tag_follow_tag:
		for tag in dict2_tag_follow_tag[prev]:
			dict2_tag_follow_tag[prev][tag]/=tag_count[prev]*1.0
			#print prev,"-",tag,"  ",dict2_tag_follow_tag[prev][tag]
...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
	...
	output_test_tags = [[] for _ in range(len(test_words))]   #list of list of predicted tags, corresponding to the list of list of words in Test set (test_words input to this function)
	#      *** WRITE YOUR CODE HERE *** 
	...
	for i in range(len(test_words)):
		...
		for j in range(len(line)):
			if line[j] not in dict2_word_tag:
				line[j]="UNK"

	for prev in dict2_tag_follow_tag:
		for tag in tag_count:
			if tag not in dict2_tag_follow_tag[prev]:
				dict2_tag_follow_tag[prev][tag]=0.0

	for word in dict2_word_tag:
		for tag in tag_count:
			if tag not in dict2_word_tag[word]:
				dict2_word_tag[word][tag]=0.0
	...
	for i in range(len(test_words)):
		...
		V = [{}]
		for tag in tag_count:
				V[0][tag] = {"prob": dict2_tag_follow_tag['*'][tag]*dict2_word_tag[line[0]][tag], "prev": None}
		for j in range(1, len(line)):
			V.append({})

			for tag in tag_count:
				max_tr_prob = max(V[j-1][prev]["prob"]*dict2_tag_follow_tag[prev][tag] for prev in tag_count)
				for prev in tag_count:
					if V[j-1][prev]["prob"]*dict2_tag_follow_tag[prev][tag] == max_tr_prob:
						max_prob =  dict2_word_tag[line[j]][tag] * max_tr_prob
						V[j][tag] = {"prob": max_prob, "prev": prev}
						break

		max_prob = max(value["prob"] for value in V[-1].values())
		previous = None
		#last tag
		for tag, value in V[-1].items():
			if value["prob"] == max_prob:
				output_test_tags[i].append(tag)
				previous = tag
				break

		# Follow the backtrack till the first observation
		for j in range(len(V)-2,-1,-1):
			output_test_tags[i].insert(0, V[j+1][previous]["prev"])
			previous = V[j+1][previous]["prev"]
